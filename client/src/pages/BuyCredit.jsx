...
const BuyCredit = () => { 
  ...
  const payment=async(credits)=>{
    try{
      if(!user){
        setShowLogin(true)
        return
     }
     const {data}=await axios.post(backendUrl+'/api/user/pay',{credits},{headers:{token}})
     if(data.success){
        ...
        {plans.map((item,index)=>(
          <div className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500' key={index}>
             ...
             <p className='mt-6'>
              ...
              <button onClick={()=>payment(item.credits)} className='w-full bg-gray-800 text-white mt-8 text-sm rounded-md py-2.5 min-w-52 cursor-pointer'>{user?'Purchase':'Get Started'}</button>
          </div>
        ))}
      </div>
